...
MEME_GUIDE = '''## 网络热梗 + 古人 PUN 段子 (装饰, 不替代主结构, 不强求密度)

reference-memes.md 里有 **5 条人工筛选的精品段子**。你的工作**不是仿写, 不是自创**, 是**挑合适的直接用**, **不改字不重写**。

### 使用规则 (必须遵守)
1. **直接抄原文嵌入**: 段子原文一字不改, 嵌进脚本合适位置
2. **不仿写, 不自创**: 你不会写段子, 不要试, 仿写质量必差
3. **不引入新人物**: 只能用库里 3 种子 (路易十六/魏忠贤/夏侯惇), 用户后续手动补
4. **不强求密度**: 库就 5 条, 用 0-5 条都行, 0 条 (主题不合适) 也 OK
5. **同一种子不重复**: 1 个脚本里同一人物最多出现 1 次 (库里 #3/#4/#5 都是路易十六, 这 3 条同一脚本最多选 1 条)
6. **场景匹配**: 自己判断哪个段子适合当前脚本主题:
   - 讲打工人 / 绩效 / KPI → 用 #1
   - 讲相亲 / 看人 / 眼光 → 用 #2
   - 讲朋友 / 义气 / 帮忙 → 用 #3
   - 讲" 想不通" / " 没思路" → 用 #4
...
### 调性边界
- 准: 反派 (魏忠贤) + 戏说古人 (路易十六/夏侯惇 - 非正史英雄)
- 不准: 悲剧英雄 (项羽/岳飞/关羽) + 革命先烈 + 受害者
- 不准: 治愈/松弛/愿你/希望 (style doc 已有, 不要复述)

### 跟 style doc 的关系
- reference-style-video.md = 主结构 (反常识开场 + 数学对比 + 数字密度)
- reference-memes.md = 装饰 (网络热梗 + 古人 PUN)
- **装饰不能挤掉主结构**: 数字密度 ≥ 12/视频, 数学对比 ≥ 2/视频, 反转 ≥ 3/视频 必须保留, 段子只在这些之间穿插
'''
...
def build_prompt(job):
    ...
    return (
        ...
        f"4. 中段每 10-15 秒一个钩子: 具体数字 / 段子化破折号 / 数学对比 / 跨学科引用 (四选一, 可叠加网络热梗或古人 PUN 段子)\n"
        f"5. 数字密度: >= {int(target_chars/100)} 个数字 (含中文) 在全文, 数学对比 >=2 个 (A 倍 / 约等于)\n"
        f"6. 反转密度: 但是/其实/真相是/实际上 类词 >=3 个\n"
        f"7. 结尾禁止: 开放式问号 / 治愈系 / 以上就是... / 希望对你有帮助\n"
        f"8. 热梗: 从 reference-memes.md 5 条里挑合适的直接用, 不改字不仿写不自创, 同一人物最多 1 次, 不强求密度 (0-5 条都行)\n"
        f"9. 写完自检: 5 个连续名词并排? 同一句式用了 2 次? 有治愈/松弛/温柔吗? 字数是否在 {min_chars}-{max_chars} 区间? 段子是否一字不改嵌入 (没仿写没自创)? 任何一项不通过就重写\n"
        f"10. 不要尝试用 N 段完整 4 层结构堆长度, 一段层只算一个反转, 4 层反转 + 中间段子 = 600-800 字就够\n\n"
